...
## Claude Code 既知の問題と一時的なガードレール

> **注意**: この対応は一時的なものです。問題の修正状況は https://github.com/anthropics/claude-code/issues/15322 を確認してください。修正されたらこのセクションは削除してください。

### UTF-8 マルチバイト文字によるパニック問題 (Issue #14133)

**背景**: Claude Code v2.0.70以降において、Rustの文字列スライス処理にバグがあり、マルチバイト文字でCLIがクラッシュする問題が発生しています。

参考: https://qiita.com/yonaka15/items/c4b95b7d9e932c9d3ff2

### 必須ガードレール

CLIのクラッシュを防ぐため、以下のルールを厳守してください:

1. **全角括弧の禁止**:
   - ❌ `（補足）` `（済）`
   - ✅ `(補足)` `(済)`
   - 説明文、TODOリスト、コミットメッセージでは必ず半角の `()` を使用すること

2. **太字フォーマットの安全対策**:
   - 太字テキストの直後にマルチバイト文字を配置しない
   - ❌ `**完了**です`
   - ✅ `**完了** です` (スペースを挿入する)

3. **TodoWrite使用時の注意**:
   - `TodoWrite`でタスクを追加する際、説明文に全角記号を使用しない

### ルール

**出力に含まれる全角括弧 `（）` は全て半角括弧 `()` に置換すること。**

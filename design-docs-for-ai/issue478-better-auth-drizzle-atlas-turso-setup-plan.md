...
| Drizzle DB クライアントと Better Auth インスタンスの最小雛形 | `.env.example` の整備（Better Auth / Turso 含む）→ **K 側で対応済み**（コミット `d1ea092`） |
| Atlas CLI のインストール手順ドキュメント化（README） | スキーマの本格定義（user/session/account/verification）→ `#479` |
| `atlas schema inspect` で空 DB のスキーマ取得確認 | Atlas マイグレーションの実運用検証 → `#479` |
| | GitHub OAuth ログイン → `#480` |
...
| 必要な環境変数（`TURSO_DATABASE_URL`、`TURSO_AUTH_TOKEN` 等）が `.env.example` に追記されている事 | **K 側で対応済み**（コミット `d1ea092`「`#478 .env.example を追加`」）。本 Issue 実装担当者の作業対象外 |

> 補足 1: `.env.example` は **K 側で対応済み**（Better Auth / Turso 関連の必要変数を含めて整備済み）。本 Issue 実装担当者は `.env.example` を編集しない。
>
...
> **注**: `.env.example` は本計画書の対象外（K 側で既にコミット `d1ea092` で追加済み）。本 Issue 実装担当者は `.env.example` を編集しない。
...
> **注**: `.env.example` の整備は K 側で対応済み（コミット `d1ea092`）のため、本 Issue 実装担当者のフェーズには含めない。`.env.local` の中身は閲覧禁止（§1.4 補足 3）。
...
- **`exclude = ["_litestream*"]`**（`env "dev"` / `env "prod"` のみ）: Turso が内部的に作成する Litestream replication engine 用テーブルを Atlas の管理対象から外す。これがないと:
  ...
  - 通常運用: ローカル `.env.local` には `stg-lgtm-cat-auth` の値が入っているため、`dotenv-cli` 等で読み込めば `--env dev` で接続できる（§6.9 参照）。
...
- Issue #478 のスコープでは `env "local"` を使う場面は §9.4 の補助確認のみ。後続 Issue（#479）で `migrate diff` の主戦場となる。
...
`.env.local` の中身は閲覧禁止（§1.4 補足 3）。以下のいずれかの方法でツール側に読み込ませて実行する。`cat .env.local` 等で内容を直接表示しない事。値にスペース・クォートを含む場合に備えて、素朴な `export $(grep ...)` パターンは避ける事。
...
> **注**: `atlas schema inspect --env local` は `env "local"` に `url` がないため失敗する。そのため Done の定義「`atlas schema inspect` が動く」確認は §9.1 の `--env dev`（接続あり）で実施する。`--env local` は `schema diff` / `migrate diff` のような **dev DB を使うコマンド** を流す用途とする。
...
- [ ] `atlas.hcl` の `env "dev"` / `env "prod"` に `exclude = ["_litestream*"]` が設定されている（Turso 内部テーブルの差分混入防止）
...
- `atlas.hcl` の `exclude = ["_litestream*"]` が `env "dev"` / `env "prod"` に入っているか（`env "local"` には不要）

...
>
> 補足 2: `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN` は既に K 側でローカル `.env.local` および Vercel ダッシュボード（全環境）に登録済み。よって本 Issue の作業範囲では「ローカルでの動作確認」のみが必要で、新規発行は行わない。
>
...
# §6.9 の `atlas schema inspect --env dev` で接続確認できる
```
...
- `env "local"`: **接続先 DB を持たない**ため、`atlas schema inspect`（引数なし）のような接続必須コマンドでは使えない。external_schema 単体の検証は `atlas schema inspect --env local --url env://src` で行う（§9.4 参照）。後続 Issue #479 で `atlas migrate diff --env local <name>` の主戦場になる。
- `env "dev"`: ローカル `.env.local` および Vercel に登録済みの `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`（`stg-lgtm-cat-auth` の値）を読み、Turso 開発用 DB に接続する。`dotenv-cli` 等で `.env.local` を読み込めば `--env dev` で接続できる（§9.1 参照）。
- **`env "prod"` は本 Issue では配置しない**: 単一の `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN` 変数を `env "dev"` と共有する設計だと、env 名と実際の接続先 DB が乖離するリスクがある（例: `TURSO_DATABASE_URL` に prod 値が入った状態で `--env dev` を実行すると prod に接続してしまう）。本 Issue は最小セットアップであり本番 DB を直接操作する必要が無いため、`env "prod"` を置かないことで誤適用の経路自体を消す。本番 DB を直接操作する必要が出る #479 以降で、`TURSO_PROD_DATABASE_URL` / `TURSO_PROD_AUTH_TOKEN` のような専用変数（または別 atlas プロファイル）とともに導入する。
- **`exclude = ["_litestream*"]`**（`env "dev"` のみ）: Turso が内部的に作成する Litestream replication engine 用テーブルを Atlas の管理対象から外す。これがないと:
  - `atlas schema inspect --env dev` の出力に `_litestream*` テーブルが混入し、Done 定義「空 DB のスキーマが正常に取得できる」を満たしにくい。
  - 後続 Issue #479 の `atlas migrate diff` で `_litestream*` テーブルを削除する SQL が誤生成され、本番適用時に DB を破壊するリスクがある。
  - 公式 Turso ガイドで明示されているプラクティス。`env "local"` は Turso へ接続しないため除外不要。
...
# これにより `env "local"` に url がなくても external_schema 単体を inspect できる。
# 公式 docs: https://atlasgo.io/atlas-schema/projects#data-source-references
atlas schema inspect --env local --url env://src
```

> **注**: `atlas schema inspect --env local` を **引数なし** で実行すると `env "local"` に接続先 `url` がないため失敗する。`--url env://src` を明示することで external_schema（Drizzle export 結果）を接続なしで取得できる。Done の定義「`atlas schema inspect` で空 DB のスキーマが取得できる事」の主検証は §9.1 の `--env dev`（実際の Turso DB 接続あり）で実施し、本セクションはあくまで補助検証として残す。
>
...
- [ ] `atlas.hcl` が配置され、`atlas schema inspect --env local --url env://src` がエラーなく実行できる（`env "local"` は接続先 `url` を持たないが、`--url env://src` を渡すことで external_schema＝Drizzle export 結果を接続なしで読み取れる）
- [ ] `atlas.hcl` の `env "dev"` に `exclude = ["_litestream*"]` が設定されている（Turso 内部テーブルの差分混入防止。`env "local"` は Turso へ接続しないため不要。`env "prod"` は本 Issue では配置しない）
- [ ] `atlas.hcl` に `env "prod"` ブロックが **存在しない** こと（`env "dev"` と同じ単一環境変数を共有すると環境名と接続先が乖離するため、本番 DB を直接操作する必要が出る #479 以降で別変数とともに導入する）
...
- `atlas.hcl` の `exclude = ["_litestream*"]` が `env "dev"` に入っているか（`env "local"` は接続なしのため不要、`env "prod"` は本 Issue では配置しない）
- `atlas.hcl` に `env "prod"` ブロックが含まれていないか（単一環境変数共有による誤適用リスクを排除する方針。#479 以降で別変数とともに導入）
